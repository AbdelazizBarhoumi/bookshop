...
import { Plus, Search, Pencil, Trash2, Package, Upload, Download, FileText, ArrowUpDown, Tags } from 'lucide-react';
...
export default function Products() {
  ...
  const [catForm, setCatForm] = useState<{ name: string; nameAr: string }>({ name: '', nameAr: '' });
  ...
  const confirmDelete = () => {
    if (deleteConfirm) {
      store.deleteProduct(deleteConfirm);
  ...
  const handleSaveCategory = () => {
    ...
    const existing = customCategories.find(c =>
      c.name.toLowerCase() === catForm.name.trim().toLowerCase() && c.id !== editingCat?.id
    );
    if (existing) { toast.error(t('inventory.categoryExists')); return; }

    let updated: CustomCategory[];
    if (editingCat) {
      updated = customCategories.map(c => c.id === editingCat.id
        ? { ...c, name: catForm.name.trim(), nameAr: catForm.nameAr.trim() || undefined }
        : c
      );
      toast.success(t('inventory.categoryUpdated'));
    } else {
      const newCat: CustomCategory = {
        id: 'cat_' + store.generateId(),
    ...
    store.saveSettings({ ...store.settings, customCategories: updated });
    setCatDialogOpen(false);
  };

  const handleDeleteCategory = () => {
    if (!deleteCatConfirm) return;
    const hasProducts = products.some(p => p.category === deleteCatConfirm);
    if (hasProducts) { toast.error(t('inventory.cannotDeleteUsed')); setDeleteCatConfirm(null); return; }
    const updated = customCategories.filter(c => c.id !== deleteCatConfirm);
    store.saveSettings({ ...store.settings, customCategories: updated });
  ...
  const handleExportPDF = () => {
    ...
    if (result instanceof Blob) {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `products-${new Date().toISOString().slice(0, 10)}.html`;
      a.click();
      URL.revokeObjectURL(url);
    } else {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `products-${new Date().toISOString().slice(0, 10)}.pdf`;
      a.click();
      URL.revokeObjectURL(url);
    }
    ...
    <div className="p-6 space-y-6 animate-slide-in">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={openAddCategory} className="gap-1">
            <Tags size={14} /> {t('inventory.manageCategories')}
          </Button>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b bg-muted/30">
                ...
                <th className={`${isRTL ? 'text-left' : 'text-right'} p-3 text-xs font-medium text-muted-foreground uppercase tracking-wider cursor-pointer select-none`} onClick={() => toggleSort('price')}>
                  <span className={`flex items-center gap-1 ${isRTL ? 'justify-start' : 'justify-end'}`}>{t('inventory.price')} <ArrowUpDown size={12} /></span>
                </th>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input type="number" step="1" min="0" value={form.price || ''} onChange={e => updateField('price', parseFloat(e.target.value) || 0)} />
              </div>
              <div>
                <Label>{t('inventory.costCurrency', { currency: 'SDG' })}</Label>
                <Input type="number" step="1" min="0" value={form.cost || ''} onChange={e => updateField('cost', parseFloat(e.target.value) || 0)} />
              </div>
              <div>
      ...
      <ConfirmDialog
        ...
        description={t('confirm.deleteProductDesc')}
      ...
      <Dialog open={catDialogOpen} onOpenChange={setCatDialogOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><Tags size={18} /> {t('inventory.manageCategories')}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {BUILTIN_CATEGORIES.map(key => (
                <div key={key} className="flex items-center justify-between p-2 rounded-lg bg-muted/30">
                  <span className="text-sm">{CATEGORY_ICONS[key]} {t(CATEGORY_I18N_KEYS[key])}</span>
                  <span className="text-xs text-muted-foreground">{t('inventory.builtInCategory')}</span>
                </div>
              ))}
              {customCategories.map(cat => (
                <div key={cat.id} className="flex items-center justify-between p-2 rounded-lg bg-muted/30">
                  <span className="text-sm">🏷️ {cat.name}{cat.nameAr ? ` (${cat.nameAr})` : ''}</span>
                  <div className="flex gap-1">
                    <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => openEditCategory(cat)}>
                      <Pencil size={12} />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={() => setDeleteCatConfirm(cat.id)}>
                      <Trash2 size={12} />
                    </Button>
                  </div>
                </div>
            ...
            <div className="border-t pt-4 space-y-3">
              <p className="text-sm font-medium">{editingCat ? t('inventory.editCategory') : t('inventory.addCategory')}</p>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <Label>{t('inventory.categoryNameAr')}</Label>
          ...
          <DialogFooter>
            <Button variant="secondary" onClick={() => setCatDialogOpen(false)}>{t('common.close')}</Button>
            <Button onClick={handleSaveCategory} disabled={!catForm.name.trim()}>
              {editingCat ? t('common.update') : t('inventory.addCategory')}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <ConfirmDialog
        open={deleteCatConfirm !== null}
        onOpenChange={(open) => !open && setDeleteCatConfirm(null)}
        title={t('inventory.editCategory')}
        description={t('inventory.cannotDeleteUsed')}
        confirmLabel={t('common.delete')}
